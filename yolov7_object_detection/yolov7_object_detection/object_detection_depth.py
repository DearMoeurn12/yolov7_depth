...
def draw_detections(image, ori_depth, boxes, scores, class_ids, mask_alpha=0.3):
    mask_img = image.copy()
    det_img = image.copy()
...
class YOLOv7:
    ...
    def draw_detections(self, image, depth, draw_scores=True, mask_alpha=0.4):

        return draw_detections(image, depth,self.boxes, self.scores,
...
class MySubscriber(Node):
    def __init__(self, model_path):
        ...
        self.yolov7_detector = YOLOv7(model_path, conf_thres=0.45, iou_thres=0.45)
    ...
    def callback(self,image,depth):
        self.get_logger().warning("Working LOL")
        try:
        ...
        combined_img = self.yolov7_detector.draw_detections(cv_image, cv_depth)
